...
**Для локальной разработки рекомендуется**: `npm run dev` (больше логов, hot reload)  
**Docker Compose**: предназначен для продакшена и деплоя в Coolify  
**Для Coolify**: Просто указывайте порт `8080` при настройке приложения.
...
- `docker-compose.yml` - универсальная конфигурация для разработки и продакшена
